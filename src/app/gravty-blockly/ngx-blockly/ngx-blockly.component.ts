  ...
  implements OnInit, AfterViewInit, OnChanges, OnDestroy
{
  ...
  @Output() public workspaceCreate: EventEmitter<Blockly.WorkspaceSvg> =
    new EventEmitter<Blockly.WorkspaceSvg>();
  @Output()
  public workspaceChange: EventEmitter<Blockly.Events.AbstractEventJson> =
    new EventEmitter<Blockly.Events.AbstractEventJson>();
  ...
  @Output() public javascriptCode: EventEmitter<string> =
  ...
  @Output() public pythonCode: EventEmitter<string> =
    new EventEmitter<string>();
        ...
        if (customBlock.blockMutator) {
          const mutator_mixin: any = {
            mutationToDom: function () {
              return customBlock.blockMutator.mutationToDom.call(
                customBlock.blockMutator,
                this
              );
            },
            domToMutation: function (xmlElement: any) {
              customBlock.blockMutator.domToMutation.call(
                customBlock.blockMutator,
                this,
                xmlElement
              );
            ...
            saveExtraState: function () {
              return customBlock.blockMutator.saveExtraState.call(
                customBlock.blockMutator
              );
            },
            loadExtraState: function (state: any) {
              customBlock.blockMutator.loadExtraState.call(
                customBlock.blockMutator,
                state
              );
            },
          ...
          if (
            ...
            customBlock.blockMutator.blockList.length > 0
          ) {
            mutator_mixin.decompose = function (workspace: any) {
              return customBlock.blockMutator.decompose.call(
                customBlock.blockMutator,
                this,
                workspace
              );
            ...
            mutator_mixin.compose = function (topBlock: any) {
              customBlock.blockMutator.compose.call(
                customBlock.blockMutator,
                this,
                topBlock
              );
            ...
            mutator_mixin.saveConnections = function (containerBlock: any) {
              customBlock.blockMutator.saveConnections.call(
                customBlock.blockMutator,
                this,
                containerBlock
              );
          ...
          Blockly.Extensions.registerMutator(
            ...
            mutator_mixin,
            function () {
              customBlock.blockMutator.afterBlockInit.call(
                customBlock.blockMutator,
                this
              );
            },
  ...
  ngAfterViewInit() {
    ...
    this.workspace = Blockly.inject(
      this.primaryContainer.nativeElement,
      this.config
    );
    ...
    for (const generator of this.config.generators) {
      switch (generator) {
        ...
        case NgxBlocklyGenerator.LUA:
          this.luaCode.emit(
            luaGenerator.workspaceToCode(Blockly.Workspace.getById(workspaceId))
          );
          break;
        case NgxBlocklyGenerator.JAVASCRIPT:
          this.javascriptCode.emit(
        ...
        case NgxBlocklyGenerator.PHP:
          this.phpCode.emit(
            phpGenerator.workspaceToCode(Blockly.Workspace.getById(workspaceId))
          );
          break;
        case NgxBlocklyGenerator.PYTHON:
          this.pythonCode.emit(
        ...
        case NgxBlocklyGenerator.XML:
          this.xmlCode.emit(
            Blockly.Xml.domToPrettyText(
              Blockly.Xml.workspaceToDom(Blockly.Workspace.getById(workspaceId))
            )
          );
          break;
      }
  ...
  public fromXml(xml: string) {
    ...
    Blockly.Xml.clearWorkspaceAndLoadFromXml(
      (Blockly.Xml as any).textToDom(xml),
      this.workspace
    );
    if (this._secondaryWorkspace) {
      Blockly.Xml.clearWorkspaceAndLoadFromXml(
        (Blockly.Xml as any).textToDom(xml),
        this._secondaryWorkspace
      );
    }
  }
  ...
  public setReadonly(readOnly: boolean) {
    ...
    if (readOnly) {
      ...
      if (!this._secondaryWorkspace) {
        ...
        this._secondaryWorkspace = Blockly.inject(
          this.secondaryContainer.nativeElement,
          config
        );
      }
      Blockly.Xml.clearWorkspaceAndLoadFromXml(
        (Blockly.Xml as any).textToDom(this.toXml()),
        this._secondaryWorkspace
      );
      Blockly.svgResize(this._secondaryWorkspace);
    } else {
